...
export const handleDailycheckApi = async (req, res, path) => {
  if (path === '/api/apps/dailycheck/today' && req.method === 'GET') {
    const data = await todayHandler();
    return json(res, data);
  }

  if (path === '/api/apps/dailycheck/answer' && req.method === 'POST') {
  ...
  if (path === '/api/apps/dailycheck/history' && req.method === 'GET') {
  ...
  if (path === '/api/apps/dailycheck/refresh' && req.method === 'POST') {
