<template>
  <div class="flex h-full w-full flex-col overflow-hidden bg-[#111]">
    <div class="flex items-center justify-between border-b border-white/10 px-4 py-2.5">
      <div class="flex items-baseline gap-1.5">
        <span class="text-sm font-bold text-white">__T_APP_TOP_CHAT__</span>
        <span v-if="contextLabel" class="text-[10px] text-white/35">{{ contextLabel }}</span>
      </div>
      <div class="flex items-center gap-2">
        <button class="cursor-pointer text-white/40 transition-colors hover:text-white" @click="newChat" title="__T_CHAT_NEW_TITLE__">
          <Plus class="h-4 w-4" />
        </button>
        <button class="cursor-pointer transition-colors" :class="showHistory ? 'text-white' : 'text-white/40 hover:text-white'" @click="toggleHistory" title="__T_HISTORY_TITLE__">
    ...
    <div v-if="showHistory" class="flex-1 overflow-y-auto [scrollbar-width:thin]">
      <div v-if="!historyList.length" class="px-4 py-8 text-center text-xs text-white/35">__T_HISTORY_EMPTY__</div>
      <button
        v-for="c in historyList"
        :key="c.conversation_id"
        type="button"
        class="flex w-full cursor-pointer items-start gap-2.5 border-b border-white/10 px-4 py-2.5 text-left transition-colors hover:bg-white/5 last:border-b-0"
      ...
      >
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-0.5 text-[10px] text-white/35">{{ c.created_at }}</div>
        </div>
      </button>
    </div>
      ...
      <div ref="msgBox" class="min-h-0 flex-1 overflow-y-auto [scrollbar-width:thin]" @scroll="onScroll">
        <div class="flex flex-col gap-0 px-3 py-3">
          <div v-if="!messages.length" class="flex flex-col items-center justify-center py-10 text-center">
            ...
            <h3 class="mb-1 text-sm font-bold text-white">__T_CHAT_EMPTY_TITLE__</h3>
            <p v-if="contextLabel" class="max-w-[260px] text-[11px] leading-relaxed text-white/35" v-html="'__T_CHAT_SIDE_CONTEXT_HINT__'.replace('{app}', `<b class=&quot;text-white/70&quot;>${contextLabel}</b>`)"></p>
            <p v-else class="max-w-[260px] text-[11px] leading-relaxed text-white/35">__T_CHAT_EMPTY_DESC__</p>
            <div v-if="quickMessages.length" class="mt-4 flex w-full flex-col gap-1.5 px-1">
              <button v-for="(msg, idx) in quickMessages" :key="idx" @click="sendQuick(msg)" class="cursor-pointer rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-left text-[11px] text-white/70 transition-colors hover:border-white/20 hover:bg-white/[0.08]">{{ msg }}</button>
            </div>
          </div>

          <template v-else>
            <div v-if="hasMore" class="py-1 text-center text-[10px] text-white/35">
              <span>__T_CHAT_LOAD_MORE__</span>
            </div>

            <div v-for="(m, i) in messages" :key="m._key || i" class="mb-3">
              <div v-if="m.role === 'user'" class="flex justify-end">
                <div class="max-w-[85%] rounded-[14px_14px_4px_14px] bg-white px-3 py-2 text-[12px] leading-relaxed text-[#111]">
                  ...
                  <div v-if="m.attachments?.length" class="mt-1.5">
                    <template v-for="(f, idx) in m.attachments" :key="idx">
                      ...
                      <div v-else-if="f.type === 'context'" class="mb-0.5 inline-block rounded-full border border-black/10 bg-black/5 px-2 py-0.5 text-[10px] text-white/50">{{ f.label }}</div>
                    </template>
                  </div>
                </div>
              </div>
              ...
              <div v-else-if="m.role === 'assistant'" class="flex items-start gap-2">
                <div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/5 text-xs">🤖</div>
                <div class="chat-md min-w-0 flex-1 overflow-x-auto rounded-[14px_14px_14px_4px] bg-white/5 px-3 py-2 text-[12px] leading-relaxed text-white/70" v-html="renderMd(m.content)" />
              </div>
              ...
              <div v-else-if="m.type === 'tool_call'" class="flex items-start gap-2">
                ...
                <div class="min-w-0 flex-1 overflow-hidden rounded-lg border border-white/10 bg-white/5">
                  <button type="button" @click="m.expanded = !m.expanded" class="flex w-full cursor-pointer items-center gap-1.5 border-none bg-white/[0.06] px-2.5 py-1.5 text-left transition-colors hover:bg-white/5">
                    <ChevronRight class="h-2.5 w-2.5 shrink-0 text-white/35 transition-transform" :class="m.expanded ? 'rotate-90' : ''" />
                    <span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-[10px] text-white/70">{{ m.title || '__T_CHAT_TOOL_CALL__' }}</span>
                    <span v-if="m.result" class="shrink-0 text-[10px] text-white/35">__T_CHAT_DONE__</span>
                  </button>
                  <div v-if="m.expanded" class="border-t border-white/10">
                    <div v-if="m.shell && m.command" class="overflow-x-auto whitespace-pre bg-white/[0.04] px-2.5 py-2 font-mono text-[10px] text-[#4ade80]"><span class="select-none text-white/35">$ </span>{{ m.command }}</div>
                    <div v-else-if="m.detail" class="overflow-x-auto whitespace-pre bg-white/[0.04] px-2.5 py-2 font-mono text-[10px] text-[#4ade80]">{{ m.detail }}</div>
                    <div v-if="m.result" class="max-h-32 overflow-auto whitespace-pre border-t border-white/10 bg-white/[0.04] px-2.5 py-2 font-mono text-[10px] text-white/35">{{ m.result }}</div>
                  </div>
                </div>
              </div>
              ...
              <div v-else-if="m.role === 'tool' || m.type === 'tool_result'" class="flex items-start gap-2">
                ...
                <div class="min-w-0 flex-1 overflow-x-auto whitespace-pre font-mono text-[10px] leading-relaxed text-white/35">{{ m.result || m.content }}</div>
              </div>
            </div>

            <div v-if="busy" class="flex items-start gap-2">
              <div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/5 text-xs">🤖</div>
              <div class="py-1 text-[12px] text-white/35">__T_CHAT_THINKING__<span class="animate-pulse">...</span></div>
            </div>
          </template>
        </div>
      </div>

      <div class="shrink-0 border-t border-white/10 px-3 pb-3 pt-2">
        <form @submit.prevent="handleSend" class="flex items-end gap-1.5 rounded-lg border border-white/10 bg-white/[0.04] pr-1">
          <textarea
            ref="textarea"
            v-model="input"
            @input="autoResize"
            ...
            rows="1"
            :disabled="busy"
            class="min-h-[36px] max-h-[100px] min-w-0 flex-1 resize-none overflow-y-auto border-none bg-transparent px-3 pb-2 pt-2 text-[12px] leading-relaxed text-white/70 outline-none placeholder:text-white/25 disabled:opacity-50"
          />
          <div class="mb-1 shrink-0">
            <button v-if="busy" type="button" @click="stopBusy" class="flex h-7 w-7 items-center justify-center rounded-md border-none bg-white text-[#111] transition-opacity hover:opacity-80">
              <Square class="h-3 w-3 fill-current" />
            </button>
            <button v-else type="submit" :disabled="!input.trim()" class="flex h-7 w-7 items-center justify-center rounded-md border transition-all" :class="input.trim() ? 'cursor-pointer border-transparent bg-white text-[#111] hover:opacity-85' : 'cursor-default border-white/10 bg-white/[0.06] text-white/35'">
