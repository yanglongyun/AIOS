...
const isAnthropicApiUrl = (apiUrl) => {
  const url = String(apiUrl || "").toLowerCase();
  return url.includes("anthropic.com") || url.includes("claude");
};

// 按 API 地址推断请求头(anthropic/claude 用 x-api-key,其余用 Bearer;openrouter 带额外头)。
const buildLlmHeaders = (apiUrl, apiKey) => {
...
const callLlmStream = async (apiUrl, apiKey, payload, { signal, onMessage } = {}) => {
  ...
  const res = await fetch(apiUrl, {
    method: "POST",
    headers: buildLlmHeaders(apiUrl, apiKey),
