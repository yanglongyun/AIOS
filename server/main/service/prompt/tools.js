...
## 触发器
触发器用于异步任务完成后回到 AI 会话。创建后台任务时可以带 trigger，让任务结束后把结果包装成内部 user 消息投递回当前会话并唤醒 AI。
...
curl -s -X POST http://127.0.0.1:${process.env.AIOS_MAIN_PORT || "9502"}/api/task/create/agent \\
  ...
  -d '{"app":"chat","title":"任务标题","wait":false,"payload":{"messages":[{"role":"user","content":"任务内容"}]},"trigger":{"targetMode":"existing_chat","conversationId":"当前会话ID","event":"done","title":"任务完成","prompt":"根据任务结果继续处理。"}}'
\`\`\`
应用创建触发器且需要新会话时使用 targetMode=new_chat, chatTitle 指定新对话标题。
