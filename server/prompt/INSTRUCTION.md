...
## 应用构建
系统提供统一应用框架，能够帮助用户创建和维护应用。
- 创建、修改、扩展应用前，先读 `memory/应用创建指导.md`
- 使用某个现有应用作为参考时，先读对应应用目录下的 `APP.md`
- 需要理解整体结构时，再读 `memory/architecture/` 下的架构文档
- 不要凭印象创建应用，具体目录规范、`APP.md` 要求、注册方式、重载流程都以创建指导为准
...
系统会先做后台预检，预检通过后才会弹出最终确认框；预检失败时会直接返回错误，不会影响当前正在运行的服务。

**用法**：发送 POST 请求到 `/api/system/reload/request`：
```bash
curl -X POST http://localhost:9500/api/system/reload/request \
  -H "Content-Type: application/json" \
  -d '{"build": true, "restartApps": true, "restartServer": false, "message": "更新了登录页面样式"}'
```

调用后，系统会先在测试端口启动临时进程做预检：
- apps 预检端口：`9511`
- server 预检端口：`9510`

只有预检通过，前端才会弹窗让用户确认；确认后系统才会执行最终切换。

**参数说明**：
- `build: true` — 修改了 `ui/` 下的前端文件时需要
- `restartApps: true` — 修改了 `apps/` 下的应用服务代码时需要
- `restartServer: true` — 修改了 `server/` 下的主服务代码并且你明确希望切换主服务时使用
- 两者都不传或都为 `false` — 仅编译前端，不重启任何服务（纯 UI 改动）
- `message` — 简述本次更新内容，显示在确认弹窗中

**注意**：
- 正常情况下只调用 `/api/system/reload/request`，不要直接调用 `/api/system/reload`。
- 如需单独验证能否启动，可调用 `/api/system/reload/test`，它只做预检，不会弹窗，也不会切换当前服务。
- 旧参数 `restart: "apps"` 仍兼容，但新代码与新文档统一使用 `restartApps` / `restartServer`。
