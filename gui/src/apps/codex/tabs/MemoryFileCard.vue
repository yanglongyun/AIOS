      ...
      <div class="ml-auto flex items-center gap-1">
        <template v-if="!editing">
          <button class="text-[11px] px-2 py-0.5 rounded-md hover:bg-black/5" style="color:#5c4332" @click="expanded = !expanded">
            {{ expanded ? '__T_CODEX_MEMORY_COLLAPSE__' : '__T_CODEX_MEMORY_EXPAND__' }}
          </button>
          <button class="text-[11px] px-2 py-0.5 rounded-md hover:bg-black/5" style="color:#5c4332" @click="startEdit">✎ __T_CODEX_MEMORY_EDIT__</button>
        </template>
        <template v-else>
          <span v-if="saveError" class="text-[10.5px]" style="color:#b03a20">{{ saveError }}</span>
          <button class="text-[11px] px-2 py-0.5 rounded-md hover:bg-black/5" style="color:#8a7965" :disabled="saving" @click="cancelEdit">__T_CODEX_MEMORY_CANCEL__</button>
          <button class="text-[11px] px-2.5 py-0.5 rounded-md cc-btn-primary font-semibold" :disabled="saving" @click="save">{{ saving ? '__T_CODEX_MEMORY_SAVING__' : '__T_CODEX_MEMORY_SAVE__' }}</button>
        </template>
      </div>
    </div>
...
const save = async () => {
  ...
  try {
    ...
    if (!data.ok) {
      saveError.value = data.error || '__T_CODEX_MEMORY_SAVE_FAILED__';
      return;
    }
    editing.value = false;
    emit('saved');
