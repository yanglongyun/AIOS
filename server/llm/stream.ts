import type { AnyRecord, ToolFunctionCall } from '../../shared/types.ts';
import { buildLlmHeaders } from './common.ts';
...
const ensureToolCall = (toolCalls: ToolFunctionCall[], index: number) => {
  if (!toolCalls[index]) {
    toolCalls[index] = {
      id: '',
      type: 'function',
...
const parseOpenAiDelta = (json: AnyRecord, state: StreamState, onDelta?: (delta: string) => void) => {
...
export const callLlmStream = async (
  ...
  const state: StreamState = { content: '', toolCalls: [] };
