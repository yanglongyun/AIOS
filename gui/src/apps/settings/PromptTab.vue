...
onMounted(async () => {
    try {
        ...
        if (data.success) {
            ...
            error.value = data.message || '__T_TASKS_LOAD_FAIL__';
        }
    } catch (e) {
...
const save = async () => {
    ...
    try {
        ...
        if (!res.ok || data.success === false) throw new Error(data.message || '__T_SETTINGS_PROMPT_SAVE_FAILED__');
        notice.value = '__T_SETTINGS_PROMPT_SAVED__';
    } catch (e) {
        error.value = e.message;
    } finally {
        saving.value = false;
