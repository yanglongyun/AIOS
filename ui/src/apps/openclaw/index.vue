<template>
  <div class="flex h-full flex-col overflow-hidden font-['Georgia','PingFang_SC',serif] bg-[#2a1e14] relative">
    ...
    <div class="flex-1 min-h-0 relative z-[2] overflow-hidden">
      ...
      <!-- 时刻表页 -->
      <div class="page absolute inset-0 flex flex-col overflow-hidden" :class="currentView === 'timetable' ? 'active' : 'hide-left'">
      ...
      <!-- 详情页 -->
      <div class="page page-bg absolute inset-0 flex flex-col overflow-hidden z-[3]" :class="currentView === 'detail' ? 'active' : (currentView === 'viz' ? 'hide-left' : 'hide-right')">
        <div class="flex shrink-0 items-center gap-2.5 px-4 pt-2 pb-1.5">
      ...
      <!-- 可视化页 -->
      <div class="page page-bg absolute inset-0 flex flex-col overflow-hidden z-[4]" :class="currentView === 'viz' ? 'active' : 'hide-right'">
        <div class="flex shrink-0 items-center gap-2.5 px-4 pt-2 pb-1.5">
...
<style scoped>
/* === 只有 Tailwind 无法表达的：复杂渐变纹理、radial-gradient、多层 shadow、伪元素、cubic-bezier 动画 === */

/* 软木板纹理 — repeating-conic-gradient + 多层 repeating-linear-gradient */
.cork-bg {
  background:
...
/* 黄铜徽章 — radial-gradient + 多层 inset shadow */
...
/* 黄铜/返回按钮 — 渐变 + text-shadow + 多层 shadow */
...
/* NOW 线 — 伪元素渐变 + radial-gradient 圆点 */
...
/* 状态圆点 — radial-gradient + glow shadow */
...
/* 表单 — border-bottom 变色 + placeholder italic */
