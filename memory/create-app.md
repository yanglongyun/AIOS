...
创建应用前先阅读目标应用的 `APP.md` 与 `memory/architecture.md`。
...
{ id: 'newapp', name: 'app_sidebar_newapp', icon: '🆕', iconClass: 'icon-newapp',
  load: () => import('./apps/newapp/index.vue'), defaultSize: { w: 800, h: 560 } },
```

应用以窗口方式打开，无需路由注册。从其他应用打开本应用用 `windowManager.open('newapp')`。
