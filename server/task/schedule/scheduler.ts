import type { TaskScheduleRow } from '../../../shared/types.ts';
import { getEnabledTaskSchedules, updateTaskScheduleById } from '../../repository/task/schedule.ts';
import { createAgentTask } from '../create/agent.ts';
import { shouldRunCron } from './cron.ts';
import { broadcast } from '../../system/ws.ts';

let timer = null;

const nowSql = (now = new Date()) => {
  const pad = (n) => String(n).padStart(2, '0');
  return `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())} ${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`;
};

const shouldRunSchedule = (schedule: TaskScheduleRow, now: Date, nowStr: string) => {
...
const tick = async () => {
  ...
  for (const s of schedules) {
    ...
    try {
      ...
      const patch: Partial<TaskScheduleRow> = {
        last_run_at: nowStr,
