      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <template v-if="view === 'history'">
            <button @click="view = 'list'" class="text-[#8b949e] hover:text-[#c9d1d9] text-[13px] font-medium transition-colors">← Back</button>
          </template>
          <template v-else>
      ...
      <div v-else class="p-4 space-y-3">
        ...
        <div v-for="repo in repos" :key="repo.id"
          ...
          <div v-if="analyzingId === repo.id && !analyses[repo.id]" class="flex items-center gap-2 mt-4 pt-4 border-t border-[#21262d] text-xs text-[#8b949e]">
            ...
            <span class="animate-pulse">Analyzing repository details...</span>
          </div>
