---
name: chat
description: 对话应用，多会话、WebSocket 流式回复、工具调用、附件上传。
...
- 后端：`server/main/api/chat/`、`server/main/service/chat/`、`server/main/ai/`、`server/main/llm/`、`server/main/service/prompt/`
- 数据：`database/aios.db` 中的 `chats`、`messages`
- 入口：
  - HTTP：`/api/chat/*`
  - WebSocket：聊天流式会话
  - 协议：`gui/src/apps/chat/protocol.js`
- 附件：`/api/fs/upload`，文件落到 `files/uploads/chat`
