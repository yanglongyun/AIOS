...
`AGENTS.md` is the project guide for Codex, Claude Code, and similar development collaborators. It is not the runtime system prompt for the iimos AI. Runtime AI instructions are stored in the database as `settings.systemPrompt` and assembled into model context by `server/main/service/prompt/index.js`.

If you are a dev collaborator, a few orientation points:

- The current working directory **is** the live iimos runtime workspace — the files you edit are the same files the `9501` (main) and `9502` (apps) Node processes are running
...
iimos has password auth enabled:
...
- AI / curl uses `Authorization: Bearer $IIMOS_API_TOKEN` (the token lives in the `auth.api_token` row and is injected into the agent runtime's environment at server startup)
