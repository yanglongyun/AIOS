...
const safeResolve = (urlPath) => {
  ...
  const target = path.normalize(path.join(UI_DIST, clean));
  // 防目录穿越:解析结果必须仍在 UI_DIST 内
  if (target !== UI_DIST && !target.startsWith(`${UI_DIST}${path.sep}`)) {
    return null;
  }
  return target;
};
