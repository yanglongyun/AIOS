<template>
  <div class="h-full overflow-y-auto cc-thin-scroll px-6 py-5 space-y-4">
    <div>
      <div class="text-[17px] font-bold">技能</div>
      <div class="text-[11.5px]" style="color:#6b5a46">来源 <span class="cc-mono">~/.claude/skills</span> + <span class="cc-mono">plugins/*/skills</span></div>
    </div>
    <div v-if="!data" class="text-[12px]" style="color:#8a7965">加载中...</div>
    <div v-else class="grid grid-cols-2 gap-4">
      <div>
        <div class="text-[11px] font-bold uppercase tracking-wider mb-2" style="color:#8a7965">{{ '用户技能 ({n})'.replace('{n}', String(data.user.length)) }}</div>
        <div class="space-y-2">
          <div v-for="s in data.user" :key="s.filePath" class="cc-card">
            <div class="flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <div class="text-[11px] mt-0.5" style="color:#6b5a46">{{ s.description || '无描述' }}</div>
              </div>
            </div>
          </div>
      ...
      <div>
        <div class="text-[11px] font-bold uppercase tracking-wider mb-2" style="color:#8a7965">{{ '插件技能 ({n})'.replace('{n}', String(data.plugins.length)) }}</div>
        <div class="space-y-2">
