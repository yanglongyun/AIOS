 ...
 *      - language/<locale>/skills/<skill>/SKILL.md 烘焙到 skills/<skill>/SKILL.md
 *      APP.md / SKILL.md 烘焙目标是 runtime,gitignored;`git clone`+`npm i` 后根目录无这些产物,
...
const REPLACE_EXTS = new Set(['.js', '.mjs', '.vue', '.json', '.md']);
const EXCLUDE_DIRS = new Set(['node_modules', '.git', '.aios', 'database', 'files', 'dist', 'language', 'scripts', 'dev']);
...
const mirrorLocalizedTree = ({ sourceRoot, targetRoot, label, keep = () => true }) => {
  let count = 0;
  if (fs.existsSync(targetRoot)) {
    fs.rmSync(targetRoot, { recursive: true, force: true });
  }
  if (!fs.existsSync(sourceRoot)) {
    console.log(`[start] mirrored 0 ${label} to ${path.relative(projectRoot, targetRoot)}/`);
    return;
  }
  ...
  console.log(`[start] mirrored ${count} ${label} to ${path.relative(projectRoot, targetRoot)}/`);
};

mirrorLocalizedTree({
  sourceRoot: path.join(langDir, 'skills'),
  targetRoot: path.join(projectRoot, 'skills'),
  label: 'skill docs',
  keep: (rel) => path.basename(rel) === 'SKILL.md',
});
