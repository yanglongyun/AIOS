...
- `language/<lang>/apps/` — 应用说明文档源资产，按语言保存。
- `apps/` — 当前语言烘焙后的应用说明文档目录，形态为 `apps/<appname>/APP.md`（只存 markdown，不是源码，不要往里写 `.js`/`.vue`）。
...
2. 参考现有应用时读当前运行态 `apps/<appname>/APP.md`；修改文档源时改 `language/<lang>/apps/<appname>/APP.md`；代码读对应的 `server/apps/<appname>/`、`gui/src/apps/<appname>/`。
...
| 层级 | 数据库表示 | 你能看到什么 |
|---|---|---|
| **必读** | `pinned=1, enabled=1` | 每次对话都自动注入**全文** |
| **星标** | `pinned=0, enabled=1` | 只注入**标题与描述**，正文需主动调 `/api/memory/get?id=<id>` 获取 |
| **已存** | `enabled=0` | 不注入，对你完全不可见，相当于暂存归档 |

`creator` 字段标记创建者：`user`（用户）、`ai`（AI）、`system`（系统）。

所以，当系统提示词里看到「以下是已星标的记忆」清单时，那只是标题摘要——如果用户的请求需要某条记忆的细节，先调 `/api/memory/get?id=<id>` 拿全文再操作。
...
如果要修改 AIOS 自己的提示词，改 `language/` 下的源文件：
- `language/zh/AGENTS.md`
- `language/en/AGENTS.md`

仓库根的 `AGENTS.md` 是 `scripts/start.mjs` 按当前 locale 烘焙出来的产物，**不要直接改**——改完源文件后重新烘焙。

提示词由主系统 prompt 装配读取，所以改完需要重新烘焙 + `restartServer: true` 才会生效。
