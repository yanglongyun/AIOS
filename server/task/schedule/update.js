...
const updateSchedule = ({ id, name, prompt, run_at, cron, enabled }) => {
  ...
  if (!runAt && !cronExpr) return { status: 400, message: "Either run_at or cron is required" };
  if (runAt && cronExpr) return { status: 400, message: "run_at and cron cannot be provided together" };
