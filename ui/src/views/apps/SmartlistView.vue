<template>
  <div class="h-full flex flex-col bg-white dark:bg-neutral-900 font-sans">
    <div class="h-16 shrink-0 border-b border-neutral-100 dark:border-neutral-800 flex items-center justify-between px-6 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md z-10">
      ...
      <div class="flex items-center gap-3">
        <input v-model="newTopic" 
               @keyup.enter="generateList"
               type="text" 
               placeholder="输入主题，回车生成..." 
               class="w-64 bg-neutral-100 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-full px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-purple-500/20" />
        <button @click="generateList" 
                :disabled="generating || !newTopic.trim()"
                class="bg-purple-600 hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-5 py-2 rounded-full text-sm font-bold transition-all flex items-center gap-2 shadow-lg active:scale-95">
          <svg v-if="!generating" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" stroke-width="3" stroke-linecap="round"/></svg>
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div class="w-72 shrink-0 border-r border-neutral-100 dark:border-neutral-800 flex flex-col bg-neutral-50/50 dark:bg-black/10">
        <div class="p-3">
          <input v-model="searchQuery" type="text" placeholder="搜索列表..." class="w-full bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-purple-500/20" />
        </div>
        <div class="flex-1 overflow-y-auto">
          <div v-for="list in filteredLists" :key="list.id" 
               @click="selectList(list)"
               :class="activeList?.id === list.id ? 'bg-purple-50 dark:bg-purple-500/10 border-purple-100 dark:border-purple-500/20' : 'hover:bg-neutral-100 dark:hover:bg-neutral-800/50 border-transparent'"
               class="mx-2 mb-1 p-3 rounded-xl cursor-pointer border transition-all group relative">
            <h3 class="text-sm font-bold text-neutral-800 dark:text-neutral-100 truncate pr-6">{{ list.topic }}</h3>
            <p class="text-xs text-neutral-400 mt-1">{{ list.item_count || 0 }} 个项目</p>
            <button @click.stop="deleteList(list.id)" class="absolute right-2 top-3 opacity-0 group-hover:opacity-100 p-1 text-neutral-400 hover:text-red-500 transition-all">
          ...
          <div v-if="lists.length === 0" class="text-center py-8 text-neutral-400 text-sm">
            暂无列表，输入主题生成
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 flex flex-col bg-white dark:bg-neutral-900">
        <div v-if="activeList" class="h-full flex flex-col">
          ...
          <div class="flex-1 overflow-y-auto p-6">
            <div v-for="item in activeList.items" :key="item.id" 
              ...
              <span class="flex-1 text-neutral-700 dark:text-neutral-200">{{ item.content }}</span>
              <button @click="deleteItem(item.id)" class="opacity-0 group-hover:opacity-100 p-1 text-neutral-400 hover:text-red-500 transition-all">
        ...
        <div v-else class="flex-1 flex flex-col items-center justify-center text-neutral-300 dark:text-neutral-600 space-y-4">
          ...
          <p class="text-sm">从左侧选择一个列表或输入主题生成</p>
        </div>
      </div>
    </div>
  </div>
