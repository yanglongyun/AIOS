    ...
    >
      ...
      <template v-else>
        <button @click="$emit('open-chat', c)" class="min-w-0 flex-1 cursor-pointer border-none bg-transparent p-0 text-left font-['Georgia','PingFang_SC',serif]">
          <div class="truncate text-sm text-[#4a3a28]">{{ c.title || c.id.slice(0, 8) }}</div>
        ...
        <div class="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
          ...
          <span v-if="deletingId === c.id" class="px-1 text-[11px] font-['Georgia','PingFang_SC',serif] text-[#c04040]">确认删除?</span>
          <button
            @click.stop="confirmDelete(c.id)"
            :title="deletingId === c.id ? '点击确认' : '删除'"
            class="flex h-7 w-7 items-center justify-center rounded-lg border-none bg-transparent text-[#b8a888] transition-all hover:bg-[#ece0c8] hover:text-[#5a4a38]"
            :class="deletingId === c.id ? 'bg-[#c04040] text-white hover:bg-[#c04040] hover:text-white' : ''">
            <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
