  ...
  <div class="relative min-h-screen overflow-hidden bg-[#f7f3ee] [font-family:'Barlow',system-ui,sans-serif] text-[#222]">
    ...
    <div class="relative z-10 flex min-h-screen items-center justify-center px-4 py-6">
      <div v-if="step === 1" class="flex w-full items-center justify-center">
        <div class="w-full max-w-[560px] overflow-hidden rounded-[28px] border border-white/80 bg-white/90 shadow-[0_20px_60px_rgba(0,0,0,0.1),0_4px_16px_rgba(0,0,0,0.06)]">
          <div class="px-6 pt-9 sm:px-10 sm:pt-10">
            <h1 class="mb-1.5 text-[28px] font-bold leading-[1.2] tracking-[-0.5px] text-[#222]">{{ t.welcome_title }}</h1>
            <p class="text-[14px] font-medium leading-[1.7] text-black/55">{{ t.welcome_subline }}</p>
          </div>

          <div class="flex items-center gap-4 px-6 pt-6 sm:px-10">
            <div class="h-px flex-1 bg-black/[0.08]"></div>
            <div class="inline-flex rounded-full bg-black/[0.05] p-[4px]">
              <button
                type="button"
                class="rounded-full px-5 py-[7px] text-[12.5px] font-semibold transition-all"
                :class="activeTab === 'api' ? 'bg-[#222] text-white' : 'text-black/50 hover:text-[#222]'"
                @click="activeTab = 'api'"
              >
                {{ t.manual_panel_label }}
              </button>
              <button
                type="button"
                class="rounded-full px-5 py-[7px] text-[12.5px] font-semibold transition-all"
                :class="activeTab === 'agent' ? 'bg-[#222] text-white' : 'text-black/50 hover:text-[#222]'"
                @click="activeTab = 'agent'"
              >
                {{ t.agents_panel_label }}
              </button>
            </div>
            <div class="h-px flex-1 bg-black/[0.08]"></div>
          </div>

          <div v-if="activeTab === 'api'" class="flex min-w-0 flex-col px-6 pb-8 pt-5 sm:px-10">
            <div class="flex min-w-0 flex-col gap-2.5">
              ...
              <div class="flex flex-col gap-[5px]">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  class="mt-[2px] self-end text-[11.5px] font-medium text-black/45 underline-offset-[3px] transition-colors hover:text-[#222] hover:underline"
                >
                  {{ t.get_key }} ↗
                </a>
              </div>
          ...
          <div v-else class="flex flex-col px-6 pb-8 pt-5 sm:px-10">
            <div class="mb-4 text-[13px] font-medium leading-[1.6] text-black/55">{{ t.agents_intro }}</div>

            <div class="mb-5 flex gap-2">
              <div
                v-for="agent in agents"
                :key="agent.name"
                class="flex min-w-0 flex-1 items-center gap-2 rounded-[10px] border border-black/[0.06] bg-white/70 px-2.5 py-[7px]"
              >
                <img :src="agent.icon" :alt="agent.name" class="h-[16px] w-[16px] shrink-0" />
                <span class="truncate text-[12px] font-semibold text-[#222]">{{ agent.name }}</span>
              </div>
            </div>

            <div class="mb-[5px] text-[10px] font-bold uppercase tracking-[0.1em] text-black/35">{{ t.agents_message_hint }}</div>
            <div
              class="mb-5 select-all whitespace-pre-wrap rounded-[10px] border border-black/[0.08] bg-black/[0.03] px-3 py-[10px] text-[12px] leading-[1.65] text-[#222] [overflow-wrap:anywhere]"
              style="font-family: 'SF Mono', 'Menlo', 'Consolas', monospace;"
            >{{ t.agents_message_template }}</div>

            <button
              type="button"
              class="inline-flex w-full items-center justify-center rounded-full border border-black/[0.12] bg-white/70 px-5 py-[10px] text-[13px] font-semibold text-black/60 transition-all hover:-translate-y-px hover:border-black/[0.22] hover:bg-white hover:text-[#222] disabled:cursor-not-allowed disabled:opacity-45 disabled:transform-none"
              :disabled="pending"
              @click="skipToDesktop"
            >
              {{ t.enter_desktop_later }}
        ...
        <div class="w-full max-w-[540px] overflow-hidden rounded-[28px] border border-white/80 bg-white/90 shadow-[0_20px_60px_rgba(0,0,0,0.1),0_4px_16px_rgba(0,0,0,0.06)]">
          <div class="flex flex-col items-start px-8 py-12 sm:px-12 sm:pb-11">
            ...
            <div class="flex w-full flex-col items-center gap-2.5 border-t border-black/[0.06] pt-6">
              <button
                class="inline-flex w-full max-w-[240px] items-center justify-between self-center rounded-full bg-[#222] py-[11px] pl-[22px] pr-[11px] text-[13.5px] font-semibold text-white shadow-[0_4px_16px_rgba(0,0,0,0.18)] transition-all hover:-translate-y-px hover:bg-[#111] hover:shadow-[0_8px_24px_rgba(0,0,0,0.24)] disabled:cursor-not-allowed disabled:opacity-45 disabled:transform-none"
                :disabled="entering"
                @click="enterSystem"
              >
                <span>{{ t.enter }}</span>
