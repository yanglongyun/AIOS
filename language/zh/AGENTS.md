...
这份文档给 Codex、Claude Code 等开发协作者阅读。它不是 AIOS 运行态 AI 的系统提示词。
...
AIOS 没有"系统级应用 vs 用户级应用"的区分 —— **所有 nav 上能进的入口都是应用,平级**。差别只在后端:
...
`/api/*`、`/apps/*`、`/files/*`、`/ws*` 全部受鉴权保护。你作为本机进程,有专属 API token,通过环境变量 `AIOS_API_TOKEN` 注入(由服务启动时从数据库读出)。
...
- 不要 `echo $AIOS_API_TOKEN`、`env | grep TOKEN` 把值打到对话里
- 不要把 token 真值复制粘贴到任何命令
- 不要把 token 写进文件、记忆、日志、提交信息

系统在写库 / 推送给前端 / 写日志前会做 token redaction(把真值替换回字面量 `$AIOS_API_TOKEN`),但这是兜底,不是免责符。
