...
const normalizeJob = (job: AnyRecord) => {
...
export const addCron = async ({ name, schedule, prompt, sessionTarget }: AnyRecord) => {
...
export const updateCron = async ({ jobId, name, schedule, prompt, sessionTarget }: AnyRecord) => {
...
export const listCronRuns = async ({ jobId, limit = 20 }: AnyRecord) => {
  const id = String(jobId || '').trim();
