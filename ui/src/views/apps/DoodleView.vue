<template>
  <div class="flex h-full flex-col bg-[#f5f0e8] font-['Georgia','PingFang_SC',serif] dark:bg-[#1a1410]">
    ...
    <div class="flex-1 overflow-y-auto">
      <div class="mx-auto max-w-[640px] px-6 py-6">

        <!-- 画布 -->
        <div class="mb-4 flex flex-col items-center">
          <div v-if="!imageSrc" class="flex flex-col items-center gap-3">
            <div class="flex h-48 w-full max-w-[400px] cursor-pointer items-center justify-center rounded-xl border-2 border-dashed border-[#dcd0b8] text-sm text-[#b8a888] transition hover:border-[#c8a060] hover:text-[#c8a060] dark:border-[#3a2a1a] dark:text-[#4a3a28]"
          ...
          <div v-else class="relative inline-block">
            <canvas ref="canvas" class="max-h-[50vh] max-w-full cursor-crosshair rounded-lg shadow-lg"
              @mousedown="startSelect" @mousemove="onDrag" @mouseup="endSelect"
              @touchstart.prevent="startSelectTouch" @touchmove.prevent="onDragTouch" @touchend="endSelect" />
        ...
        <!-- 指令 -->
        <div class="mb-4">
          <div class="mb-2 text-xs font-semibold text-[#8a7a60] dark:text-[#6a5840]">修改指令</div>
          <textarea v-model="prompt" placeholder="例如：把选中区域的猫变成狗" rows="2"
        ...
        <div v-if="selection" class="mb-4 rounded-lg border border-[#dcd0b8] bg-[rgba(200,160,96,0.04)] p-3 text-xs text-[#8a7a60] dark:border-[#3a2a1a]">
          选区: {{ Math.round(selection.x) }}%, {{ Math.round(selection.y) }}% → {{ Math.round(selection.x + selection.w) }}%, {{ Math.round(selection.y + selection.h) }}%
        </div>

        <!-- 结果 -->
        <div v-if="editResult" class="mb-4">
...
const getTouchPos = (e, rect) => {
  const t = e.touches[0];
  return { x: (t.clientX - rect.left) / rect.width * 100, y: (t.clientY - rect.top) / rect.height * 100 };
};

const startSelect = (e) => {
  dragStart = getPos(e, canvas.value.getBoundingClientRect());
};

const startSelectTouch = (e) => {
  dragStart = getTouchPos(e, canvas.value.getBoundingClientRect());
};

const onDrag = (e) => {
  if (!dragStart) return;
  const p = getPos(e, canvas.value.getBoundingClientRect());
  selection.value = { x: Math.min(dragStart.x, p.x), y: Math.min(dragStart.y, p.y), w: Math.abs(p.x - dragStart.x), h: Math.abs(p.y - dragStart.y) };
  updateSelectionStyle();
};

const onDragTouch = (e) => {
  if (!dragStart) return;
  const p = getTouchPos(e, canvas.value.getBoundingClientRect());
  selection.value = { x: Math.min(dragStart.x, p.x), y: Math.min(dragStart.y, p.y), w: Math.abs(p.x - dragStart.x), h: Math.abs(p.y - dragStart.y) };
  updateSelectionStyle();
};
