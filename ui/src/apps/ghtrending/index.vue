          ...
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <a :href="repo.url" target="_blank" class="text-[#58a6ff] text-sm font-semibold hover:underline truncate">{{ repo.name }}</a>
              </div>
              ...
              <div class="flex items-center gap-4 text-[11px] text-[#484f58]">
                ...
                <span class="flex items-center gap-0.5">⭐ {{ fmtNum(repo.stars) }}</span>
                <span class="flex items-center gap-0.5">🍴 {{ fmtNum(repo.forks) }}</span>
              </div>
            </div>
            <button @click="analyzeRepo(repo)" :disabled="analyzingId === repo.id"
              ...
              {{ analyzingId === repo.id ? '...' : analyses[repo.id] ? '✦' : '✦ __T_GH_ANALYZE__' }}
            </button>
          </div>
          <!-- Inline analysis -->
...
const repos = ref([]); const loading = ref(false); const since = ref('daily'); const language = ref('');
...
const loadRepos = async () => { loading.value = true; try { repos.value = (await (await fetch(`/aios/apps/ghtrending/list?since=${since.value}&language=${language.value}`)).json()).repos || []; } catch {} loading.value = false; };

const analyzeRepo = async (repo) => {
  if (analyses[repo.id]) { delete analyses[repo.id]; return; }
  analyzingId.value = repo.id;
  try { analyses[repo.id] = (await (await fetch('/aios/apps/ghtrending/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ repo, locale: LOCALE }) })).json()).analysis || ''; }
  catch { analyses[repo.id] = 'Failed'; } analyzingId.value = null;
};
