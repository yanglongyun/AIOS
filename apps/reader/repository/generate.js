...
const countChapters = (sessionId) => {
  const { c } = db.prepare(
    "SELECT COUNT(*) as c FROM reader_chapters WHERE session_id = ?"
  ).get(sessionId);
  return c || 0;
};
const insertChapter = ({ sessionId, idx, action, content, choicesJson, summary, progress }) => {
  db.prepare(`
...
const updateSessionProgress = ({ sessionId, summary, progress, chapterCount }) => {
  db.prepare(`
