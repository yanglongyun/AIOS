...
export const handleSystemApi = async (req, res, path) => {
  ...
  if (path === '/api/system/reload' && req.method === 'POST') {
    ...
    try {
      ...
      // 3) 重启主服务 (port 9700) — 必须最后执行，会退出当前进程
      if (restart === 'server' || restart === 'both') {
        setTimeout(() => {
          const child = spawn('sh', ['-c', 'sleep 1 && exec node server/index.js'], {
            cwd: ROOT_DIR,
            detached: true,
            stdio: 'ignore',
            env: { ...process.env }
