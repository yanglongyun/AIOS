      ...
      <div ref="msgBox" class="min-h-0 flex-1 overflow-y-auto [scrollbar-width:thin] [&::-webkit-scrollbar-thumb]:rounded [&::-webkit-scrollbar-thumb]:bg-black/10 [&::-webkit-scrollbar]:w-1.5 dark:[&::-webkit-scrollbar-thumb]:bg-white/10" @scroll="onScroll">
        <div class="mx-auto flex max-w-[720px] flex-col gap-0 px-5 py-6">
          ...
          <div v-if="!messages.length" class="flex flex-1 flex-col items-center justify-center py-20 text-center">
            ...
            <p v-if="context" class="max-w-[320px] text-[13px] leading-relaxed text-[#a0907a] dark:text-[#6a5840]" v-html="t('chat_side_context_hint', { app: `<b class=&quot;text-[#5a4a38] dark:text-[#d4c0a0]&quot;>${context}</b>` })"></p>
            <p v-else class="max-w-[320px] text-[13px] leading-relaxed text-[#a0907a] dark:text-[#6a5840]">{{ t('chat_empty_desc') }}</p>
          </div>
...
const handleSend = async () => {
  ...
  // 上下文注入：第一条消息带上当前应用信息
  let content = text || t('chat_attachment_only_prompt');
...
// 监听 conversationId prop 变化（由外部路由驱动）
watch(() => props.conversationId, async (id) => {
  ...
  try {
    ...
    if (e?.status === 404) { resetState(); emit('not-found'); return; }
    messages.value.push({ role: 'assistant', content: t('chat_send_error', { message: e.message }) });
  }
}, { immediate: true });
