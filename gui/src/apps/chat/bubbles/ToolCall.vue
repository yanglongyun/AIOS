  ...
  <div class="max-w-[720px] overflow-hidden rounded-xl border border-line bg-white">
    ...
    <div v-if="msg.expanded" class="border-t border-line/60">
      <pre v-if="msg.shell && msg.command"
        class="m-0 overflow-x-auto whitespace-pre-wrap break-words bg-[#f6f8fc] px-3 py-2.5 text-[12px] leading-[1.55] text-good"><span class="select-none text-faint">$ </span>{{ msg.command }}</pre>
      <pre v-else-if="msg.detail"
        class="m-0 overflow-x-auto whitespace-pre-wrap break-words bg-bg-elev px-3 py-2.5 text-[12px] leading-[1.55] text-muted">{{ msg.detail }}</pre>
      <pre v-if="msg.result"
        class="m-0 max-h-[200px] overflow-y-auto whitespace-pre-wrap break-words border-t border-line/60 bg-accent/3 px-3 py-2.5 text-[12px] leading-[1.55] text-muted">{{ msg.result }}</pre>
    </div>
  </div>
</template>
