  ...
  <div class="composer-wrap">
    ...
    <div class="composer">
      ...
      <textarea
        ...
        :value="modelValue"
        rows="1"
        :placeholder="streaming ? '执行中...' : '输入指令，Enter 发送'"
        :disabled="streaming"
      ...
      <button v-if="streaming"
        class="grid h-9 w-9 place-items-center rounded-md border-0 bg-bad text-white transition-colors hover:opacity-85"
        title="中断"
        @click="emit('abort')">
        <span class="msi sm !text-white">stop</span>
      </button>
      <button v-else
        class="grid h-9 w-9 place-items-center rounded-md border-0 bg-accent text-bg-elev transition-colors hover:bg-accent-hi disabled:cursor-default disabled:opacity-40"
        :disabled="!canSend"
        @click="emit('send')">
