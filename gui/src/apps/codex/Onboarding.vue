  ...
  <div class="h-full overflow-y-auto cc-thin-scroll">
    <div class="mx-auto max-w-[720px] px-8 py-12 space-y-8">
      <div>
        ...
        <div class="text-[24px] font-bold">__T_CODEX_ONBOARDING_TITLE__</div>
        <p class="mt-3 text-[13.5px] leading-[1.7]" style="color:#4a3826">
          __T_CODEX_ONBOARDING_DESC__
        </p>
        <p class="mt-2 text-[12px]" style="color:#6b5a46">__T_CODEX_ONBOARDING_HINT__</p>
      </div>

      <div class="space-y-5">
        <div class="text-[13px] font-bold uppercase tracking-wider" style="color:#8a7965">__T_CODEX_ONBOARDING_INSTALL_TITLE__</div>
        <div v-for="cmd in commands" :key="cmd.os" class="rounded-[14px] overflow-hidden" style="border:1px solid rgba(140,100,60,0.16);background:#fff">
          <div class="flex items-center justify-between px-4 py-2.5 border-b" style="border-color:rgba(140,100,60,0.10);background:#fdf7e8">
            ...
            <button class="text-[11px] px-2.5 py-1 rounded-md cc-btn-primary font-semibold" @click="copy(cmd)">
              {{ copied === cmd.os ? '__T_CODEX_ONBOARDING_COPIED__' : '__T_CODEX_ONBOARDING_COPY__' }}
            </button>
          </div>
      ...
      <div class="flex items-center gap-3">
        <button class="cc-btn-primary rounded-lg px-4 py-2 text-[13px] font-semibold" @click="$emit('recheck')">
          {{ checking ? '__T_CODEX_ONBOARDING_CHECKING__' : '__T_CODEX_ONBOARDING_RECHECK__' }}
        </button>
        <a href="https://github.com/openai/codex" target="_blank" rel="noopener" class="text-[12px] underline" style="color:#5c4332">__T_CODEX_ONBOARDING_GITHUB__</a>
      </div>
    </div>
  </div>
</template>
...
const commands = [
  { os: '__T_CODEX_ONBOARDING_NPM__', icon: '📦', cmd: 'npm install -g @openai/codex' },
  { os: '__T_CODEX_ONBOARDING_HOMEBREW__', icon: '🍎', cmd: 'brew install codex' }
];

const copied = ref(null);
let timer = null;
const copy = async (cmd) => {
  try { await navigator.clipboard.writeText(cmd.cmd); } catch {}
  copied.value = cmd.os;
  clearTimeout(timer);
