  ...
  <Teleport to=".cc-app-root" :disabled="!open">
    ...
    <div class="flex flex-col overflow-hidden w-full max-w-[920px]"
      ...
      <div class="flex items-center gap-3 px-4 py-3 border-b" style="border-color:rgba(140,100,60,0.14);background:#fffaf2">
        <div class="min-w-0 flex-1">
          ...
          <div v-if="data?.ok" class="text-[10.5px] cc-mono" style="color:#8a7965">
            {{ formatSize(data.size) }}<span v-if="data.truncated"> · __T_CODEX_FILE_TRUNCATED__</span>
          </div>
          <div v-else-if="loading" class="text-[10.5px]" style="color:#8a7965">__T_CODEX_LOADING__</div>
        </div>
      ...
      <div class="flex-1 overflow-auto cc-thin-scroll">
        <div v-if="loading" class="p-6 text-[12px]" style="color:#8a7965">__T_CODEX_FILE_READING__</div>
        <div v-else-if="!data?.ok" class="p-6 text-[12px]" style="color:#b03a20">{{ data?.error || '__T_CODEX_FILE_LOAD_FAILED__' }}</div>
