...
- 浏览器侧统一以 `/aios/` 为 public base
- 系统 API 使用 `/aios/api/*`
- 应用 API 使用 `/aios/apps/*`
- 不要在前端写裸路径 `/api/*` 或 `/apps/*`

错误示例：

```js
await fetch('/apps/todo/list');
```

正确示例：

```js
const API_BASE = '/aios/apps/todo';
const res = await fetch(`${API_BASE}/list`);
```

如果前端把请求发到 `/apps/*` 或 `/api/*`，开发服务器可能返回一段 base URL 提示文本，而不是 JSON。此时前端继续按 JSON 解析，就会出现 `Unexpected token ...` 一类错误。
...
1. 调用 `/api/system/reload/request`
2. 系统会先做后台预检
3. 预检通过后才弹最终确认框
4. 用户确认后，系统再执行最终切换
...
如果只是想验证能不能启动，而不想弹窗或切换当前服务，可以调用：
...
> 系统没有 i18n 机制，界面文字直接按用户当前语言输出（由系统设置中的 language 决定），不要使用翻译函数或占位符。
