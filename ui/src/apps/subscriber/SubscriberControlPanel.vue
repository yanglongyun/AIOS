<template>
  <div>
    <div class="mb-3 flex shrink-0 items-center justify-center">
      <div class="text-[10px] font-black uppercase tracking-[0.3em] text-[#a08040]" style="text-shadow:0 1px 0 rgba(0,0,0,0.3);">__T_SUBSCRIBER_TITLE__</div>
    </div>
    ...
    <div class="dial-strip">
      <div class="shrink-0 text-center" :class="{ 'status-receiving': refreshing }">
        ...
        <div class="na-value mt-0.5 text-[14px] font-black text-[#e8c060]" style="text-shadow:0 0 10px rgba(232,192,96,0.5);">
          {{ refreshing ? '__T_SUBSCRIBER_REFRESHING__' : scheduleTimeStr }}
        </div>
      </div>
      ...
      <button class="flex shrink-0 items-center bg-transparent p-1 text-[#806040] transition-colors duration-200 hover:text-[#e8c060]" @click="$emit('toggle-settings')" title="__T_SUBSCRIBER_FOCUS_LABEL__">
    ...
    <div class="expand-panel" :class="{ 'is-open': showSettings }">
      <div class="control-panel mt-3">
        <div class="mb-3">
          <div class="mb-1.5 text-[9px] font-black tracking-widest text-[#8a7a50]">__T_SUBSCRIBER_FOCUS_LABEL__</div>
          <div class="speaker-grille-input">
            <textarea :value="focus" rows="2" class="grille-textarea" placeholder="__T_SUBSCRIBER_FOCUS_PLACEHOLDER__" @input="$emit('update:focus', $event.target.value)"></textarea>
          </div>
        </div>
        <div class="mb-4">
          <div class="mb-1.5 text-[9px] font-black tracking-widest text-[#8a7a50]">__T_SUBSCRIBER_SCHEDULE_LABEL__</div>
