      ...
      <template v-else>
        ...
        <div v-else class="flex flex-wrap items-start gap-4">
          <div v-for="it in filteredItems" :key="it.id" class="w-[calc(50%-8px)]">
            <MemoryCard :item="it" @edit="openEditor(it)" @set-visibility="(v) => setVisibility(it, v)" @delete="deleteItem(it)" />
          </div>
        </div>
      </template>
    </div>
...
const full = computed(() => items.value.filter(i => i.visibility === 'full'));
const summary = computed(() => items.value.filter(i => i.visibility === 'summary'));
const count = computed(() => items.value.filter(i => i.visibility === 'count'));
const canSave = computed(() => editTitle.value.trim() && editContent.value.trim());

const tabs = computed(() => [
  { key: 'all', label: '全部', count: items.value.length },
  { key: 'full', label: '全部可见', count: full.value.length },
  { key: 'summary', label: '仅摘要', count: summary.value.length },
  { key: 'count', label: '仅数量', count: count.value.length },
]);

const filteredItems = computed(() => {
  if (tab.value === 'full') return full.value;
  if (tab.value === 'summary') return summary.value;
  if (tab.value === 'count') return count.value;
  return items.value;
});
...
const setVisibility = async (item, visibility) => {
  ...
  await post('/api/memory/update', { id: item.id, visibility });
