...
const serializeMessagesForSummary = (rows = []) => rows.map((row) => {
  ...
  if (role === "assistant" && Array.isArray(message.tool_calls) && message.tool_calls.length) {
    content = [message.content || "", `tool_calls: ${JSON.stringify(message.tool_calls)}`].filter(Boolean).join("\n");
  } else if (role === "tool") {
  ...
  try {
    const result = await callLlmStream(settings.apiUrl, settings.apiKey, {
      ...
      messages: [
        ...
        { role: "user", content: `请压缩以下聊天消息：\n\n${serializeMessagesForSummary(candidates)}` },
      ],
    }, { signal });
    const summary = String(result.message?.content || "").trim();
    if (!summary) return null;

    const id = createCompaction({ chatId, startMessageId, endMessageId, summary, tokens: readTotalTokens(result.usage) });
    ...
    const meta = { kind: "compaction", startMessageId, endMessageId, tokens: compaction.tokens, compaction };
    const messageId = appendMessage(chatId, message, { ...meta, source: "user" });
