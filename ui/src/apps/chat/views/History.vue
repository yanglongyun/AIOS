<template>
  <div class="absolute inset-0 flex flex-col overflow-hidden" :style="bgStyle">
    ...
    <div class="flex-1 overflow-y-auto pb-4 [scrollbar-width:none]">
      ...
      <template v-else>
        <div class="px-[18px] pt-3 pb-1.5 text-[11px] font-semibold uppercase tracking-widest text-[#9a7850]" style="font-family:system-ui,sans-serif">{{ t('chat_history_recent', '最近') }}</div>
        <button
          v-for="c in chats"
          :key="c.id"
          ...
          <div class="rounded-[14px] overflow-hidden" :style="cardStyle">
            <div class="flex items-center gap-3 px-4 py-3">
              ...
              <div class="flex-1 min-w-0">
                <div class="text-[14.5px] font-bold text-[#3a2415] truncate">{{ c.title || t('chat_new_title', '新对话') }}</div>
