<template>
  <div class="flex h-full min-h-0 flex-col bg-[#faf8f4]">
    ...
    <div v-if="loading" class="flex h-full items-center justify-center text-[12.5px]" style="color:rgba(0,0,0,0.4)">__T_MEMORY_READING__</div>
...
const loadFile = async () => {
  ...
  errorMessage.value = '';
  try {
    const data = await request(`/api/memory/read?path=${encodeURIComponent(props.path)}`);
