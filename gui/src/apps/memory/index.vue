<template>
  <div class="relative flex h-full min-h-0 flex-col" style="background:repeating-linear-gradient(#faf6ef 0px,#faf6ef 31px,rgba(170,150,120,0.08) 31px,rgba(170,150,120,0.08) 32px),linear-gradient(135deg,#faf5eb,#f5efe3);color:#2a1f13">
    <!-- Red binding lines -->
    ...
    <div class="relative z-[2] flex h-14 shrink-0 items-center justify-between border-b-2 pl-14 pr-5" style="border-color:rgba(170,150,120,0.1)">
      ...
      <div class="flex items-center gap-1.5">
        ...
        <button class="rounded-[20px] border-2 border-dashed px-[18px] py-[7px] text-[17px] font-semibold transition-all hover:scale-[1.03] hover:border-[rgba(140,120,80,0.5)] hover:bg-[rgba(255,255,255,0.8)]" style="border-color:rgba(140,120,80,0.3);background:rgba(255,255,255,0.5);color:rgba(100,75,40,0.6);font-family:'Caveat','Segoe Print',cursive;backdrop-filter:blur(4px)" @click="openEditor(null)">+ __T_MEMORY_CREATE__</button>
      </div>
    </div>
    ...
    <div class="relative z-[2] min-h-0 flex-1 overflow-y-auto py-5 pl-[52px] pr-4 [scrollbar-width:thin]">
      ...
      <div v-else-if="!items.length" class="flex flex-col items-center px-6 py-16 text-center">
        ...
        <div class="mb-1 text-[22px] font-bold" style="color:rgba(80,55,30,0.5);font-family:'Caveat','Segoe Print',cursive">__T_MEMORY_EMPTY_TITLE__</div>
        <div class="mb-5 text-[16px]" style="color:rgba(140,120,90,0.4);font-family:'Caveat','Segoe Print',cursive">__T_MEMORY_EMPTY_DESC__</div>
        <button class="rounded-[20px] border-2 border-dashed px-5 py-2 text-[17px] font-semibold transition-all hover:scale-[1.03] hover:bg-[rgba(255,255,255,0.8)]" style="border-color:rgba(140,120,80,0.3);background:rgba(255,255,255,0.5);color:rgba(100,75,40,0.6);font-family:'Caveat','Segoe Print',cursive" @click="openEditor(null)">__T_MEMORY_CREATE_FIRST__</button>
      </div>

      <!-- Cards grid -->
      <template v-else>
        <div v-if="!filteredItems.length" class="flex flex-col items-center px-6 py-14 text-center">
          <div class="text-[16px] font-semibold" style="color:rgba(80,55,30,0.4);font-family:'Caveat','Segoe Print',cursive">__T_MEMORY_FILTER_EMPTY__</div>
        </div>
      ...
      <div v-if="editorOpen" class="absolute inset-0 z-30 flex items-center justify-center px-5 py-6" style="background:rgba(60,45,25,0.25);backdrop-filter:blur(8px)" @click.self="closeEditor">
        <div class="relative flex w-full max-w-[500px] flex-col overflow-hidden rounded-[6px]" style="background:#fef9e8;box-shadow:0 20px 60px rgba(60,40,10,0.2),0 0 0 1px rgba(0,0,0,0.04);max-height:85%;background-image:repeating-linear-gradient(0deg,transparent 0px,transparent 27px,rgba(0,0,0,0.025) 27px,rgba(0,0,0,0.025) 28px)">
          <div class="flex items-center justify-between px-6 pb-3 pt-5" style="border-bottom:1px solid rgba(170,150,120,0.12)">
            <div class="text-[22px] font-bold" style="color:rgba(80,55,30,0.7);font-family:'Caveat','Segoe Print',cursive">{{ editId ? '__T_MEMORY_EDIT_TITLE__' : '__T_MEMORY_NEW_TITLE__' }}</div>
          ...
          <div class="flex min-h-0 flex-1 flex-col gap-3.5 overflow-y-auto px-6 py-4 [scrollbar-width:thin]">
            <div>
              <div class="mb-1 text-[14px] font-semibold" style="color:rgba(120,100,60,0.45);font-family:'Caveat','Segoe Print',cursive">__T_MEMORY_FIELD_TITLE__</div>
              <input ref="titleInput" v-model="editTitle" placeholder="__T_MEMORY_TITLE_PLACEHOLDER__" class="w-full rounded-md border-[1.5px] px-3.5 py-2.5 text-[22px] font-bold outline-none transition-all focus:border-[rgba(170,150,100,0.4)] focus:shadow-[0_0_0_3px_rgba(170,150,100,0.08)]" style="border-color:rgba(170,150,100,0.2);background:rgba(255,255,255,0.6);color:rgba(60,40,15,0.85);font-family:'Caveat','Segoe Print',cursive" @keydown.meta.s.prevent="saveEdit" @keydown.ctrl.s.prevent="saveEdit" />
            </div>
            <div>
              <div class="mb-1 text-[14px] font-semibold" style="color:rgba(120,100,60,0.45);font-family:'Caveat','Segoe Print',cursive">__T_MEMORY_FIELD_DESC__</div>
              <input v-model="editDescription" placeholder="__T_MEMORY_DESC_PLACEHOLDER__" class="w-full rounded-md border-[1.5px] px-3.5 py-2 text-[17px] outline-none transition-all focus:border-[rgba(170,150,100,0.4)] focus:shadow-[0_0_0_3px_rgba(170,150,100,0.08)]" style="border-color:rgba(170,150,100,0.2);background:rgba(255,255,255,0.6);color:rgba(60,40,15,0.85);font-family:'Caveat','Segoe Print',cursive" @keydown.meta.s.prevent="saveEdit" @keydown.ctrl.s.prevent="saveEdit" />
              <div class="mt-1 text-[12px]" style="color:rgba(140,120,80,0.3);font-family:'Caveat','Segoe Print',cursive">__T_MEMORY_DESC_HINT__</div>
            </div>
            <div class="flex min-h-0 flex-1 flex-col">
              <div class="mb-1 flex items-center justify-between">
                <div class="text-[14px] font-semibold" style="color:rgba(120,100,60,0.45);font-family:'Caveat','Segoe Print',cursive">__T_MEMORY_FIELD_CONTENT__</div>
                <div class="text-[12px] tabular-nums" style="color:rgba(140,120,80,0.25);font-family:'Caveat',cursive">{{ editContent.length }} __T_MEMORY_CHAR_UNIT__</div>
              </div>
              <textarea v-model="editContent" placeholder="__T_MEMORY_CONTENT_PLACEHOLDER__" class="w-full flex-1 resize-y rounded-md border-[1.5px] px-3.5 py-2.5 text-[16px] leading-[1.7] outline-none transition-all focus:border-[rgba(170,150,100,0.4)] focus:shadow-[0_0_0_3px_rgba(170,150,100,0.08)]" style="border-color:rgba(170,150,100,0.2);background:rgba(255,255,255,0.6);color:rgba(60,40,15,0.85);font-family:'Caveat','Segoe Print',cursive;min-height:180px" @keydown.meta.s.prevent="saveEdit" @keydown.ctrl.s.prevent="saveEdit"></textarea>
              <div class="mt-1 text-[12px]" style="color:rgba(140,120,80,0.28);font-family:'Caveat','Segoe Print',cursive">__T_MEMORY_CONTENT_HINT__</div>
            </div>
          </div>
          <div class="flex items-center justify-end gap-2 px-6 pb-5 pt-3" style="border-top:1px solid rgba(170,150,120,0.1)">
            <button class="rounded-[20px] border-[1.5px] px-[18px] py-[7px] text-[16px] font-semibold transition-all hover:bg-[rgba(255,255,255,0.8)]" style="border-color:rgba(170,150,100,0.2);background:rgba(255,255,255,0.5);color:rgba(100,80,50,0.5);font-family:'Caveat','Segoe Print',cursive" @click="closeEditor">__T_COMMON_CANCEL__</button>
            <button class="rounded-[20px] border-none px-[22px] py-[8px] text-[16px] font-bold transition-all hover:translate-y-[-1px] disabled:opacity-30" style="background:rgba(120,85,45,0.75);color:#fef6e0;box-shadow:0 2px 6px rgba(100,70,30,0.2);font-family:'Caveat','Segoe Print',cursive" :disabled="!canSave || saving" @click="saveEdit">{{ saving ? '__T_MEMORY_SAVING__' : '__T_MEMORY_SAVE__' }}</button>
          </div>
        </div>
      </div>
    </Transition>
...
const tabs = computed(() => [
  { key: 'all', label: '__T_MEMORY_TAB_ALL__', count: items.value.length },
  { key: 'pinned', label: '__T_MEMORY_TAB_PINNED__', count: pinned.value.length },
  { key: 'enabled', label: '__T_MEMORY_TAB_ENABLED__', count: enabled.value.length },
  { key: 'disabled', label: '__T_MEMORY_TAB_DISABLED__', count: disabled.value.length },
]);
...
const deleteItem = async (item) => {
  if (!confirm(`__T_MEMORY_DELETE_CONFIRM_PREFIX__${item.title}__T_MEMORY_DELETE_CONFIRM_SUFFIX__`)) return;
